import datetime
import logging
from homeassistant.helpers import entity_registry, area_registry, event
...
async def async_setup_energy_advisor(hass):
    ...
    async def analyze_energy_usage(now):
        ...
        er = await EntityRegistry.async_get_registry(hass)
        ar = hass.helpers.area_registry.async_get(hass)
        ...
        history = hass.components.history
        suggestions = []
        ...
        for area_id, devices in areas.items():
            ...
            for light in devices["lights"]:
                states = await history.get_significant_states(start_time, end_time, entity_ids=[light])
            ...
            for motion in devices["motion"]:
                states = await history.get_significant_states(start_time, end_time, entity_ids=[motion])
