...
from .oasirapersonsensor import OasiraPersonSensor
...
class OasiraPerson(SensorEntity, RestoreEntity):
    ...
    async def async_set_notification_devices(
        ...
        existing = next(
            (d for d in self._notification_devices if d.unique_id == f"oasira_notify_{device_name}"),
            None,
        )
        if existing:
            _LOGGER.info("[OasiraPerson] Notification device %s already exists for %s", token, self._email)
        else:
            device = oasiranotificationdevice(self.hass, token, device_name, platform_name)
            self._notification_devices.append(device)
            _LOGGER.info("[OasiraPerson] Added notification device for %s: %s", self._email, platform_name)
            await self.async_update_ha_state()
        ...
        if (last_state := await self.async_get_last_state()) is not None:
            ...
            if restored_devices:
                self._notification_devices = [
                    oasiranotificationdevice(self.hass, None, dev_id, "restored")
                    for dev_id in restored_devices
                ]

            _LOGGER.info(
                "[OasiraPerson] Restored state for %s: local=%s remote=%s devices=%s",
                self._email,
